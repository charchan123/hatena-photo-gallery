...
def _knowledge(master, source_map):
    ...
    add("学名", scientific.get("value"), scientific,
        note="資料記載名" if scientific.get("name_status") == "source_reported" else None)
...
def _format_published(value):
    ...
    return f"{date.year}年{date.month}月{date.day}日公開"
...
def render_detail_sections(view):
    ...
    if articles:
        ...
        for article in articles:
            ...
            items.append(f'<li class="subject-record-item"><time class="subject-record-date">{date}</time>'
                         f'<div class="subject-record-title">{title_html}</div></li>')
        parts.append('<section class="subject-records"><h2>📔 このキノコが登場した観察記録</h2>'
                     f'<ul class="subject-record-list">{"".join(items)}</ul></section>')
    return "".join(parts)
