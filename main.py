...
galleries.forEach(gallery => {
    ...
    srcs.forEach(src => {
      ...
      sortedGroups.forEach(group => {
        ...
        group.items.forEach(src => {
            const caption = buildNoteCaption(src, "キノコ名"); // ← alt を渡す
  ...
  // 観察ノート専用のキャプション関数
  // =========================
  function buildNoteCaption(src, alt){
    ...
    return `
      <div class="exif-wrap">
        <div class="exif-title">${alt}</div>
        ${date ? `<div class="exif-middle">${date}</div>` : ""}
      </div>
    `;
  }
