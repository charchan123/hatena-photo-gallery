...
// =========================
    lg.on("lgAfterOpen", () => {
      ...
      wrap.innerHTML = `
        ...
        <button class="meta-comment-btn">💬 コメント</button>
      `;
    
      container.appendChild(wrap);
    
      const favBtn = wrap.querySelector(".meta-fav-btn");
      const commentBtn = wrap.querySelector(".meta-comment-btn");
    
      function refresh() {
        const src = lg.getCurrentSlide()?.getAttribute("href");
        if (!src) return;
