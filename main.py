...
/* =========================================================
   ★ ギャラリー本体（Animated / Justified + Masonry）
========================================================= */
.gallery {
  max-width: 1200px;
  margin: 0 auto;
...
(function () {
  ...
  // =========================
    ...
    function applyGallerySizes() {
      const items = document.querySelectorAll(".gallery a.gallery-item");
      const COLS = window.innerWidth <= 580 ? 4 : 5;
    
      let colCursor = 0;
    
      items.forEach((item, i) => {
        item.classList.remove("size-1","size-2-h","size-2-v","size-4");
    
        let span = 1;
        let cls = "size-1";
    
        if (i % 11 === 0 && colCursor <= COLS - 2) {
          cls = "size-4";
          span = 2;
        } else if (i % 5 === 0 && colCursor <= COLS - 2) {
          cls = "size-2-h";
          span = 2;
        } else if (i % 7 === 0) {
          cls = "size-2-v";
  ...
  // =========================
    ...
    function applyMasonryRows() {
      const items = document.querySelectorAll(".gallery a.gallery-item");
      const gap = 4; // CSSのgapと同じ
    
      items.forEach(item => {
        const height = item.getBoundingClientRect().height;
        const rowSpan = Math.ceil((height + gap) / 1);
