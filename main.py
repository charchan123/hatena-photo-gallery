...
# ====== iframe 高さ調整 + Masonry縦2列＋レスポンシブ1列（iframe幅対応 完全版） ======
...
.gallery img { 
  width:100%; border-radius:8px; transition:opacity 0.5s ease-out; 
...
(function() {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (gallery) {
      ...
      const setMasonryLayout = () => {
        const galleryWidth = gallery.clientWidth;  // ← iframe内の実幅で判定
        const isMobile = galleryWidth <= 320;       // ← スマホっぽい幅なら1列
        const columnWidth = isMobile 
          ? galleryWidth 
          : (galleryWidth - gutter) / 2;

        gallery.style.maxWidth = isMobile 
          ? "100%" 
  ...
  document.addEventListener("click", e => {
    ...
    if (href.startsWith("javascript:history.back") || href.startsWith("#") || 
