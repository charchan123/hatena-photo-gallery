...
// =========================
    ...
    function openCommentDialog(src) {
  ...
  function injectMetaButtons(lgInstance) {
  const toolbar = document.querySelector(".lg-toolbar");
  if (!toolbar) return;

  // 二重追加防止
  if (toolbar.querySelector(".lg-meta-wrap")) return;

  const wrap = document.createElement("div");
  wrap.className = "lg-meta-wrap";
  ...
  wrap.innerHTML = `
    <button class="lg-meta-fav" title="お気に入り">☆</button>
    <button class="lg-meta-comment" title="コメント">💬</button>
  `;

  toolbar.appendChild(wrap);

  const favBtn = wrap.querySelector(".lg-meta-fav");
  const commentBtn = wrap.querySelector(".lg-meta-comment");

  function refresh() {
    const slide = lgInstance.getCurrentSlide();
    if (!slide) return;
    const src = slide.getAttribute("href");
    const meta = getPhotoMeta(src);
    favBtn.textContent = meta.favorite ? "★" : "☆";
  }
