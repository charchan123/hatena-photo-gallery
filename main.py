...
/* ★ LightGallery メタアイコン状態制御 */
.lg-meta-fav,
.lg-meta-comment {
  color: #aaa !important;       /* 初期グレー */
  opacity: 0.8;
}

.lg-meta-active {
  color: #fff !important;       /* アクティブは白 */
  opacity: 1;
}

/* hover / active で色が戻るのを防ぐ */
...
// ⭐ お気に入り状態の見た目制御
// =========================
function refreshFavoriteState(btn, src) {
  const meta = getPhotoMeta(src);
  btn.classList.toggle("lg-meta-active", !!meta.favorite);
  btn.innerHTML = meta.favorite ? "★" : "☆";
}

// =========================
// 💬 コメント状態の見た目制御
// =========================
function refreshCommentState(btn, src) {
  const meta = getPhotoMeta(src);
  btn.classList.toggle("lg-meta-active", !!meta.comment);
}

// =========================
// 状態更新専用関数
// =========================
function updateMetaIconState() {
  const slide = document.querySelector(".lg-current .lg-object");
  if (!slide) return;

  const src = slide.getAttribute("src");
  const meta = getPhotoMeta(src);

  const favBtn = document.querySelector(".lg-meta-fav");
  const cmtBtn = document.querySelector(".lg-meta-comment");

  if (favBtn) {
    favBtn.classList.toggle("lg-meta-active", !!meta.favorite);
    favBtn.innerHTML = meta.favorite ? "★" : "☆";
  }

  if (cmtBtn) {
    cmtBtn.classList.toggle("lg-meta-active", !!meta.comment);
  }
}

// =========================
// ★ LightGallery toolbar に ⭐ 💬 を後付け（安全）
// =========================
(function attachMetaButtonsSafely() {

  // すでに監視中なら二重起動しない
  if (window.__lgMetaObserver) return;
  ...
  const observer = new MutationObserver(() => {
    ...
    // ⭐ がすでにあれば何もしない
    if (toolbar.querySelector(".lg-meta-fav")) return;

    // --- ⭐ お気に入り ---
    const favBtn = document.createElement("button");
    favBtn.className = "lg-icon lg-meta-fav";
    favBtn.title = "お気に入り";
    favBtn.innerHTML = "☆";
    ...
    favBtn.onclick = () => {
      ...
      const meta = getPhotoMeta(src);
      updatePhotoMeta(src, { favorite: !meta.favorite });
    
      refreshFavoriteState(favBtn, src);
    };

    // --- 💬 コメント ---
    const commentBtn = document.createElement("button");
    commentBtn.className = "lg-icon lg-meta-comment";
    commentBtn.title = "コメント";
    commentBtn.innerHTML = "💬";

    commentBtn.onclick = () => {
      toggleCommentUI(); // ← 次段階で実装
      const slide = document.querySelector(".lg-current .lg-object");
      if (!slide) return;
      ...
      const meta = getPhotoMeta(src);
      const text = prompt("この写真のメモ（自分用）", meta.comment || "");
      if (text !== null) {
        updatePhotoMeta(src, { comment: text });
      }
      if (text !== null) {
      updatePhotoMeta(src, { comment: text });
      refreshCommentState(commentBtn, src);
      }
    };

    // ツールバーの左側に追加（順番は後で調整できる）
    toolbar.prepend(commentBtn);
    toolbar.prepend(favBtn);
...
if (slide) {
  ...
  window.__lgMetaObserver = observer;
})();
