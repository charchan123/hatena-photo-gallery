...
.note-intro__lead{
  margin: 0 0 8px;
  font-size: 14px;      /* “説明”に見えないサイズ */
  line-height: 1.75;    /* 詰めない＝余裕＝空気 */
  opacity: 0.78;        /* 薄くして主役を写真に */
}

.note-intro__hint{
  margin: 0;
  font-size: 13px;      /* さらに一段引く */
  line-height: 1.7;
  opacity: 0.62;
}
...
/* スマホは少しだけ読みやすく */
@media (max-width: 520px){
  .note-intro{ padding: 0 12px; }
  .note-intro__lead{ font-size: 14px; opacity: 0.82; }
  .note-intro__hint{ font-size: 13px; opacity: 0.68; }
}
...
.note-divider{
  border: none;
  height: 1px;
  background: rgba(0, 0, 0, 0.08); /* 薄さ命 */
  max-width: 920px;
...
/* 正方形を作る */
.season-grid .gallery-item {
  aspect-ratio: 1 / 1;        /* ← これが肝 */
  overflow: hidden;
}

/* 中の画像はトリミング */
.season-grid .gallery-item img {
  width: 100%;
  height: 100%;
...
.lg-sub-html .note-exif {
  animation: noteFadeUp 0.35s ease both;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // =========================
    function showFavToast(message, anchorEl) {
      ...
      toast.style.left = scrollX + rect.left + rect.width / 2 + "px";
      toast.style.top = scrollY + rect.top - 32 + "px";
        ...
        star.addEventListener("click", e => {
          ...
          updateCardFavorites();
            ...
            if (favs[src]) {
              showFavToast(
                `📓 観察ノートに追加<br>
                 <span class="fav-count" data-count="${count - 1}">
                   （いま ${count - 1}件）
                 </span>`,
                star
              );
              ...
              requestAnimationFrame(() => {
                const el = document.querySelector(".fav-toast .fav-count");
                if (!el) return;
            
                el.textContent = `（いま ${count}件）`;
