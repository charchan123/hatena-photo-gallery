...
document.addEventListener("DOMContentLoaded", () => {
  ...
  commentPanel.innerHTML = `
    <div class="lg-comment-header">この写真のメモ</div>
    <div class="lg-comment-close">×</div>
    <textarea placeholder="このキノコの特徴や場所などのメモを書けます"></textarea>
  `;
  ...
  favOverlay.innerHTML = `
    <div class="lg-fav-inner">
      ...
      <div class="lg-fav-empty" style="display:none;">まだお気に入りが登録されていません。</div>
    </div>
  `;
      ...
      lgInstance.on("lgAfterOpen", () => {
        ...
        if (!toolbar.querySelector(".lg-custom-share")) {
          ...
          shareBtn.addEventListener("click", () => {
            ...
            if (navigator.share) {
              navigator.share({
                title: document.title || "フォトギャラリー",
                url: shareUrl
              }).catch(()=>{});
            } else {
              // 簡易メニュー：X / LINE / コピー
              const msg = "共有方法を選んでください：\\n1: Xに投稿\\n2: LINEで共有\\n3: URLをコピー";
        ...
        if (!toolbar.querySelector(".lg-custom-favlist")) {
          ...
          favListBtn.textContent = "⭐";
          favListBtn.title = "お気に入り一覧";
