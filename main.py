...
function sendHeight() {
  ...
  if (searchInput && cards.length) {
    ...
    function updateEmptyState() {
      const hasQuery =
        searchInput.value.trim() !== "" || currentKana !== "all";
      ...
      const visible = Array.from(cards).some(
  ...
  .querySelector(".section .search-empty");
  ...
  if (indexSearchInput && indexResults) {
    ...
    function doSearch() {
      ...
      renderPagination(rawQ ? totalPages : 0); // ← 後述
      updateURL(rawQ, page);
      sendHeight();
    }
