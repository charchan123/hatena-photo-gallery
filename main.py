...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (gallery) {
    ...
    imagesLoaded(gallery, () => {
      ...
      // favorite-gallery でも LightGallery を起動する（ここ重要）
  ...
  // =========================
    ...
    srcs.forEach(src => {
      ...
      if (info) {
        const season = getSeason(info.month);
        const seasonKey = `${info.year}-${season}`;
    
        // --- 季節が切り替わったら見出しを挿入 ---
        if (seasonKey !== lastSeasonKey) {
          gallery.appendChild(
            createSeasonHeader(info.year, season)
          );
      ...
      a.innerHTML = `
        ...
        <img src="${src}" loading="lazy" class="visible">
      `;
