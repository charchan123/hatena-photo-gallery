...
(function () {
  ...
  if (gallery) {
    ...
    // UI生成（1回だけ）
    function ensureMetaUI(container) {
      if (container.querySelector(".photo-meta-wrap")) return;
    
      const wrap = document.createElement("div");
      wrap.className = "photo-meta-wrap";
      wrap.style.marginTop = "12px";
      wrap.style.textAlign = "center";
    
      wrap.innerHTML = `
        <button class="meta-fav-btn">☆ お気に入り</button>
        <button class="meta-comment-btn" style="margin-left:8px;">💬 コメント</button>
      `;
    ...
    // 表示更新
    function refreshMetaUI(lg) {
      const slide = lg.getCurrentSlide();
      if (!slide) return;
      ...
      const favBtn = document.querySelector(".meta-fav-btn");
      if (favBtn) {
    ...
    lg.on("lgAfterOpen", () => {
      ...
      ensureMetaUI(container);
    
      const favBtn = container.querySelector(".meta-fav-btn");
      const commentBtn = container.querySelector(".meta-comment-btn");
      ...
      commentBtn.onclick = () => {
        ...
        const current = getPhotoMeta(src).comment || "";
        const text = prompt("この写真のコメント（自分用）", current);
