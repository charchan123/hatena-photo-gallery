...
/* ================================
   LightGallery カスタム Autoplay ボタン
   （見た目は他のアイコンと同化）
================================ */
.lg-custom-autoplay {
  position: relative;
...
/* Ⅱ アイコン（再生中） */
.lg-custom-autoplay.lg-autoplay-playing::before,
.lg-custom-autoplay.lg-autoplay-playing::after {
  content: "";
  position: absolute;
  top: 50%;
  width: 2px;
  ...
  if (gallery) {
    ...
    imagesLoaded(gallery, () => {
      ...
      const lg = lightGallery(gallery, {
        ...
        zoom: true,
        thumbnail: true,
        autoplay: false   // 最初は停止状態（ボタンでON/OFF）
      });
      ...
      function stopAutoplay() {
        ...
        if (autoplayBtn) {
          autoplayBtn.classList.remove("lg-autoplay-playing");
        }
      }
      ...
      function setupAutoplayButton() {
        ...
        // すでに作ってあれば何もしない
        if (toolbar.querySelector(".lg-custom-autoplay")) {
          autoplayBtn = toolbar.querySelector(".lg-custom-autoplay");
          return;
        }

        autoplayBtn = document.createElement("button");
        autoplayBtn.type = "button";
        autoplayBtn.className = "lg-icon lg-custom-autoplay";
        autoplayBtn.setAttribute("aria-label", "スライドショー再生/一時停止");
        ...
        autoplayBtn.addEventListener("click", () => {
          ...
          if (autoplay) {
            // ▶ → Ⅱ に切り替え
            autoplayBtn.classList.add("lg-autoplay-playing");
        ...
        // 先頭に挿入 → [Autoplay][Zoom][Share][Close] の並びに近づける
        toolbar.insertBefore(autoplayBtn, toolbar.firstChild);
      }

      // ギャラリーが開いたタイミングでツールバーが確実に存在する
      lg.on("lgAfterOpen", () => {
        setupAutoplayButton();
      });
