...
<script>
(function() {
  // LightGallery が読み込まれるまで監視
  const waitLG = setInterval(() => {
    const lg = document.querySelector(".lg-toolbar");
    if (!lg) return;

    clearInterval(waitLG);
    ...
    lg.prepend(btn); // 先頭に追加

    // --- 前回の状態を復元 ---
    let visible = localStorage.getItem("mushroom_exif_visible") !== "0";

    function apply() {
      const sub = document.querySelectorAll(".lg-sub-html");
      sub.forEach(el => {
        el.style.display = visible ? "block" : "none";
      });
      localStorage.setItem("mushroom_exif_visible", visible ? "1" : "0");
    }
    ...
    // スライド切替ごとに反映
    document.addEventListener("lgAfterSlide", apply);
